...
class RiceBranching(data.Dataset):

    def __init__(self, args, train=True):
        self.imagefolder = os.path.join(args.data_directory, 'rice-branching/')
        self.datafile = os.path.join(args.cache_directory,'rice-branching.pt')
        self.train = train
        self.args = args
        ...
        self.channelcount = 3
        self.channelthresholds = [ 0.2, 0.2, 0.2 ]
        self.classcount = 0
    ...
    def __getitem__(self, index):
        ...
        ordertwo = label['ordertwo'].clone()
        orderthree = label['orderthree'].clone()
        ...
        # Crop centre choice
        fn, sn, o2n, o3n = 0, 0, 0, 0
        if fertile.dim() > 0: fn = fertile.size(1)
        if sterile.dim() > 0: sn = sterile.size(1)
        if ordertwo.dim() > 0: o2n = ordertwo.size(1)
        if orderthree.dim() > 0: o3n = orderthree.size(1)
        ...
        # 60% of the time, chose a random branch point
            ...
            if sbx == 0:
                ...
                if spikeidx <= o2n:
                    centre = ordertwo[spikeidx].clone()
                else:
                    centre = orderthree[spikeidx - o2n].clone()
        ...
        if fn > 0: transformed_fertile = mat.mm(torch.cat([fertile,torch.ones(fertile.size(0))],1).t()).t().ceil()[:,0:2]
        else: transformed_fertile = None
        if sn > 0: transformed_sterile = mat.mm(torch.cat([sterile,torch.ones(sterile.size(0))],1).t()).t().ceil()[:,0:2]
        else: transformed_sterile = None
        if o2n > 0: transformed_ordertwo = mat.mm(torch.cat([ordertwo,torch.ones(ordertwo.size(0))],1).t()).t().ceil()[:,0:2]
        else: transformed_ordertwo = None
        if o3n > 0: transformed_orderthree = mat.mm(torch.cat([orderthree,torch.ones(orderthree.size(0))],1).t()).t().ceil()[:,0:2]
        else: transformed_orderthree = None
        ...
        if use_augmentation and flip:
            ...
            if transformed_ordertwo is not None:
                transformed_ordertwo[:,0].mul_(-1).add_(output_dim - 1)

            if transformed_orderthree is not None:
                transformed_orderthree[:,0].mul_(-1).add_(output_dim - 1)
        ...
        ordertwoGTPoints = self._capture_inbounds(transformed_ordertwo, output_dim)
        orderthreeGTPoints = self._capture_inbounds(transformed_orderthree, output_dim)

        hm = torch.zeros(self.channelcount, output_dim, output_dim);
        render_heatmap_2d(hm[0], transformed_fertile, gaussian_sd) # Currently same output heatmap
        render_heatmap_2d(hm[0], transformed_sterile, gaussian_sd) # Currently same output heatmap
        render_heatmap_2d(hm[1], transformed_ordertwo, gaussian_sd)
        render_heatmap_2d(hm[2], transformed_orderthree, gaussian_sd)

        return {
            'input': to_tensor(pil_image),
            'heatmap': hm,
            'scale': world_base_length,
            'groundtruth': [fertileGTPoints + sterileGTPoints, ordertwoGTPoints, orderthreeGTPoints]
        }
        ...
        if os.path.isfile(jsonpath):
            ...
            for value in data:
                ...
                if n == 'Fertile':
                    ...
                    sterile.append(p)
                elif n == '2nd Ord. Junction':
                    ordertwo.append(p)
                elif n == '3rd Ord. Junction':
                    orderthree.append(p)
            ...
            return {
                ...
                "ordertwo": torch.FloatTensor(ordertwo),
                "orderthree": torch.FloatTensor(orderthree)
            }
